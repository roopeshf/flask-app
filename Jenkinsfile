...
pipeline {
  ...
  stages {
    ...
    stage('Deploy the image using ansible') {
          ...
          ansiblePlaybook disableHostKeyChecking: true, extras: '--etra_vars=tag_var = ${BUILD_NUMBER}', installation: 'ansible', inventory: '/etc/ansible/hosts', limit: 'ansible-node', playbook: 'docker_manage.yml'  
